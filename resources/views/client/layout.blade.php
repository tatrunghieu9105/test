...
<head>
    ...
    <title>@yield('title', 'Rạp phim')</title>
  ...
  <body>
    <header>
      <div class="nav container">
        <div class="brand">
          <div class="logo"></div>
          <a href="{{ route('movies.index') }}" style="text-decoration:none; color:var(--text); display: flex; align-items: center; gap: 10px;">
            <strong>Rạp phim</strong>
          </a>
        </div>
        <nav style="display: flex; align-items: center; gap: 20px;">
          <a href="{{ route('movies.index') }}" style="text-decoration: none; color: var(--text); padding: 8px 12px; border-radius: 6px;" 
          ...
          @auth
            <a href="{{ route('me.orders') }}" style="text-decoration: none; color: var(--text); padding: 8px 12px; border-radius: 6px;"
               onmouseover="this.style.backgroundColor='rgba(99, 102, 241, 0.1)'" 
               onmouseout="this.style.backgroundColor='transparent'">
              Đơn hàng
            </a>
            ...
            <div style="position: relative; display: inline-block;">
              ...
              <div style="position: absolute; right: 0; background: var(--surface); border: 1px solid var(--border); border-radius: 8px; padding: 8px 0; min-width: 200px; margin-top: 8px; display: none; z-index: 1000; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06);">
                <div style="padding: 8px 16px; border-bottom: 1px solid var(--border);">
                  <div style="font-weight: 600;">{{ auth()->user()->fullname ?? auth()->user()->name }}</div>
                  <div style="font-size: 0.9em; color: var(--muted);">{{ auth()->user()->email }}</div>
                </div>
                <a href="{{ route('me.profile') }}" style="display: block; padding: 10px 16px; text-decoration: none; color: var(--text);"
                   onmouseover="this.style.backgroundColor='rgba(99, 102, 241, 0.1)'" 
                   onmouseout="this.style.backgroundColor='transparent'">
                  Hồ sơ của tôi
                </a>
          ...
          @else
            <a class="btn btn-outline" href="{{ route('login') }}" style="text-decoration: none;">
              Đăng nhập
            </a>
            <a class="btn" href="{{ route('register') }}" style="text-decoration: none;">
              Đăng ký
            </a>
          @endauth
        </nav>
    ...
    <footer class="container">
      <div>© {{ date('Y') }} Rạp phim • <span class="muted">Xem phim cùng bạn</span></div>
    </footer>
    @yield('scripts')
    @if(auth()->check())
    <script src="{{ asset('js/account-status.js') }}"></script>
    @endif
  </body>
  </html>
